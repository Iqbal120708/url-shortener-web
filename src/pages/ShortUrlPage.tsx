...
import { getUrlList, deleteUrl, postUrl } from '../api/url';
...
export default function ShortUrlPage() {
    ...
    const fetchUrls = useCallback((url?: string) => {
        if (loading) return;
        setLoading(true);
        getUrlList(url)
            .then((res) => {
    ...
    return (
        <div className="max-w-3xl mx-auto px-4 py-6">
            ...
            {initialLoading && <p className="text-sm text-neutral-500">Loading...</p>}
            ...
            {items.length > 0 && (
                <table className="hidden md:table w-full text-sm border-collapse">
                    ...
                    <tbody>
                        {items.map((u) => (
                            <tr key={u.id} className="border-b border-neutral-100 group">
                                <td className="py-3 pr-4 truncate max-w-xs text-neutral-700" title={u.original_url}>
                                    {u.original_url}
                                </td>
                                <td className="py-3 pr-4 text-black font-medium">
                                    {SHORT_BASE.replace(/^https?:\/\//, '')}/{u.short_code}
                                </td>
                                <td className="py-3 pr-4">
                                    <button
                                        onClick={() => navigate(`/links/${u.id}/analytics`)}
                                ...
                                <td className="py-3 text-right space-x-2">
                                    <button onClick={() => handleCopy(u)} className="p-1.5 rounded hover:bg-neutral-100" aria-label="Copy">
                                        {copiedId === u.id ? <Check size={16} /> : <Copy size={16} />}
                                    </button>
                                    <button onClick={() => handleDelete(u)} className="p-1.5 rounded hover:bg-neutral-100" aria-label="Delete">
                                        <Trash2 size={16} />
                                    </button>
                                </td>
                            </tr>
            ...
            <div className="md:hidden space-y-3">
                {items.map((u) => (
                    <div key={u.id} className="border border-neutral-200 rounded-lg p-3">
                        <div className="flex items-center justify-between">
                            <span className="font-medium text-black truncate">
                                {SHORT_BASE.replace(/^https?:\/\//, '')}/{u.short_code}
                            </span>
                            <button onClick={() => handleCopy(u)} className="p-2 -mr-2 rounded hover:bg-neutral-100" aria-label="Copy">
                                {copiedId === u.id ? <Check size={18} /> : <Copy size={18} />}
                            </button>
                        </div>
                        <p className="text-sm text-neutral-500 truncate mt-1">{u.original_url}</p>
                        <div className="flex justify-between items-center mt-2">
                            ...
                            <button onClick={() => handleDelete(u)} className="p-2 -mr-2 rounded hover:bg-neutral-100 text-neutral-500" aria-label="Delete">
                                <Trash2 size={16} />
                            </button>
                        </div>
                    </div>
            ...
            <div ref={sentinelRef} className="h-8 flex items-center justify-center">
                {loading && !initialLoading && <span className="text-xs text-neutral-400">Loading more...</span>}
            </div>
        </div>
    );
}